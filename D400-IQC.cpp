...
#ifdef _WIN32
HANDLE gStopThreadEvent, gStartTestingEvent, gSaveImageEvent;
#else
...
void testingThreadFun() {
	...
	handle[1] = gStopThreadEvent;

	while (true)
	{
		auto res = WaitForMultipleObjects(2, handle, FALSE, INFINITE);
...
struct stream_format_idx {
	...
	void reset() {
		depthSIdx = -1; infra1SIdx = -1; infra2SIdx = -1; infra1ColorFIdx = -1; infra1Y8FIdx = -1;
	}
};
...
void saveResult(test_result* result)
{
	...
	fileStream << "Accuracy," << result->accuracy << ",FillRate," << result->fillRate << ",subpixel RMS," << result->rmsSubpixel << ",fitting plane RMS," << result->rmsFittingPlane << "\n";
...
bool calTestResult(test_result result)
{
	...
	{
		if (result.fillRate[i] * 100.0f < gConfig.fillRatePassP[i])
			return false;
		if (result.mean[i] < gConfig.minDist || result.mean[i] > gConfig.maxDist)
			return false;
	}*/
	...
	if (result.rmsSubpixel > gConfig.rmsSubpixelPassRate)
		return false;
	/*if (result.rms > gConfig.rmsPassRate)
		return false;*/
...
float* calRMS(const std::vector<rs2::float3>& points, rs2::plane p,
	float baseline_mm, float focal_length_pixels, float plane_fit_to_ground_truth_mm) {
	...
	result[snapshot_metrics::FITTING_RMS] = static_cast<float>(std::sqrt(plane_fit_err_sqr_sum / distances.size()));
	return result;
...
bool calPlane(const uint16_t* data, int w, int h, float units, rs2_intrinsics* intrin, std::array<float3, 4>* corners, std::vector<rs2::float3>* _roi_pixels, plane* _p) {
...
bool calPlane(const uint16_t* data, int w, int h, float units, rs2_intrinsics* intrin, std::array<float3, 4>* corners)
{
	return calPlane(data, w, h, units, intrin, corners, nullptr, nullptr);
}
...
snapshot_metrics analyze_depth_image(const uint16_t* data, int w, int h, float units, float baseline_mm, rs2_intrinsics* intrin)
{
	...
	calPlane(data, w, h, units, intrin, corners, roi_pixels, p);
	...
	result.data = calRMS(*roi_pixels, *p, baseline_mm, intrin->fx, plane_fit_to_gt_dist_mm);
	result.p = *p;
...
int main(int, char**) try
{
    ...
    bool is_3d_view = false, paused = false, refresh_device_list=true, isPreview = false, showResult = false, nextRoundPlay = false, showTestCondition = false, editable = false/*hasInited = false*/;
    ...
    mouse_info mouse;
#ifdef _WIN32
	std::thread* saveThread[3] = { nullptr, nullptr, nullptr };
	std::thread* calResultThread = nullptr;
#else
	pthread_t saveThread[3] = { NULL, NULL, NULL };
        ...
        if (device_models.size() > 0)
        {
					...
					for (auto&& f : sub->formats)
					{
						if (sfIdx.depthSIdx == -1)
						{
							if (sub->stream_display_names[f.first] == "Depth")
							{
								...
								sub->stream_enabled[sfIdx.depthSIdx] = false;//initial depth is enabled, disable it first.
							}
						}
...

			{
				...
				if (showResult)
				{
					...
					ImGui::Text("Fitting Plane RMS (mm):");
					ImGui::SetCursorPosX(col2);
			...
			if(device_models.size() != 0)
            {
                ...
                {
					auto sub = device_models[0].subdevices[0];
                    try
                    {
                        static float t = 0.f;
                        t += 0.03f;
						...
						if (!isPreview && !gStartTesting)
						{
							...
							if (ImGui::Button("Preview", { 100, 80 }))
							{
								...
								sub->stream_enabled[sfIdx.infra1SIdx] = true;
								if (sfIdx.infra1ColorFIdx != -1)
									sub->ui.selected_format_id[sfIdx.infra1SIdx] = sfIdx.infra1ColorFIdx;
								else
									sub->ui.selected_format_id[sfIdx.infra1SIdx] = sfIdx.infra1Y8FIdx;
								showResult = false;
								isPreview = true;
								...
								{
									mViewer_model.streams[profile.unique_id()].dev = sub;
								}
							}
						}
						else {
						...
						else
						{
							...
							if (ImGui::Button("Test", { 100, 80 })) {
								if(isPreview)
								{
									sub->stop();//stop preview stream and go through the process to clean the stream.
									std::this_thread::sleep_for(std::chrono::milliseconds(300));
									isPreview = false;	
								}
								gCalculate_sts = -1;
								gStartTesting = true;
								sub->stream_enabled[sfIdx.depthSIdx] = true;
								sub->stream_enabled[sfIdx.infra2SIdx] = true;
								sub->ui.selected_format_id[sfIdx.infra1SIdx] = sfIdx.infra1Y8FIdx;//change the infrared1 format from RGB to Y8
								auto profiles = sub->get_selected_profiles();
							...
							if (sub->streaming && !isPreview)//test is running
							{
								if (!gStartTesting)//test complete
								{
									sub->stop();
									std::this_thread::sleep_for(std::chrono::milliseconds(300));
									sub->stream_enabled[sfIdx.depthSIdx] = false;
									sub->stream_enabled[sfIdx.infra2SIdx] = false;
									showResult = true;
								}
							}
						}
...
#pragma endregion UI
		...
		if (gStartTesting && calWPC)
		{
			...
			mViewer_model.update_3d_camera(layout[DUMMY_PC_STREAM_ID], mouse, true);
			
			if (auto _frame = mViewer_model.streams[sfIdx.depthSIdx].texture->get_last_frame())
			{
				video_frame vframe = _frame.as<video_frame>();
				calPlane((const uint16_t*)vframe.get_data(), vframe.get_width(), vframe.get_height(), 0.001, &intrin, &(mViewer_model.roi_rect));
			}
			...
			if (gStartCapture)
			{
				...
				if (frame)
				{
					...
					if (stream == sfIdx.depthSIdx)
					{
						...
						frameData[saveImgidx]._points = new points(p);
