...
namespace rs2
{
	...
	//need to add one more dummy stream for pointcloud view
    std::map<int, rect> viewer_model::calc_layout(const rect& r, bool calWPC)
    {
		...
		{
			//dummy index = 55
			stream_model pc_dummy;
			pc_dummy.size = { 1280, 720 };
			stream_index[&pc_dummy] = DUMMY_PC_STREAM_ID;
			active_streams.insert(&pc_dummy);
		}
	...
	void viewer_model::display_3d_view(const rect& viewer_rect)
	{	
		//glBindTexture(GL_TEXTURE_2D, texture2d);
		//glEnable(GL_TEXTURE_2D);
		glBegin(GL_QUADS);
		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		glVertex2f(viewer_rect.x, viewer_rect.y);
		glVertex2f(viewer_rect.x, viewer_rect.y + viewer_rect.h);
		glVertex2f(viewer_rect.x + viewer_rect.w, viewer_rect.y + viewer_rect.h);
		glVertex2f(viewer_rect.x + viewer_rect.w, viewer_rect.y);
		glEnd();
	...
	texture_buffer* viewer_model::upload_frame(frame&& f, points* p)
    {
		...
		if (f.get_profile().format() == RS2_FORMAT_Z16 || f.get_profile().format() == RS2_FORMAT_DISPARITY16)
		{
			if(applyFilter)
			  f = apply_filters(f);
			*p = pc.calculate(f);
